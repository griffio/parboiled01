...
@Test
public class ExpressionParserTest extends ParboiledTest<ExpressionParserTest> {
  ...
  public void addTwo() {
    ExpressionParser parser = Parboiled.createParser(ExpressionParser.class);
    test(parser.Input(), "+2").hasErrors(
        "Invalid input '+', expected Input (line 1, pos 1):\n+2\n^\n");
  }

  public void twoAddTwoAdd() {
    ExpressionParser parser = Parboiled.createParser(ExpressionParser.class);
    test(parser.Input(), "2+2+").hasErrors(
        "Unexpected end of input, expected Multiplication (line 1, pos 5):\n2+2+\n    ^\n");
  }

  public void twoMultiplyTwoMultiply() {
    ExpressionParser parser = Parboiled.createParser(ExpressionParser.class);
    test(parser.Input(), "2*2*").hasErrors(
        "Unexpected end of input, expected Number (line 1, pos 5):\n2*2*\n    ^\n");
  }
  ...
  public void valueOfThreePlusTwoMultiplyFiveAddFour() {
    ExpressionParser parser = Parboiled.createParser(ExpressionParser.class);
    ParsingResult<ValueOp> result =
        new ReportingParseRunner<ValueOp>(parser.Input()).run("3+2*5+4");
    ValueOp op = result.parseTreeRoot.getValue();
